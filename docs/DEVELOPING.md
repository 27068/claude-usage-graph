...
Work happens on `dev`. **`main` is what has been released** — it moves only at
step 6 below, and only ever forward. Everything on it is therefore already
published, which turns "do not rewrite published history" from a rule someone
has to remember into something the branch layout enforces: a push to `main` that
is not a fast-forward is the alarm, not an inconvenience to force through.

Rewriting `dev` is free and expected. That is the whole point of the split —
"this unit is finished" is a judgement made at the moment you are least able to
make it, and being wrong should cost an amend rather than a rewrite of something
public. A release is an event rather than a judgement, so it is the safe place
to put the irreversible step.

**That freedom rests on `dev` having a single writer, and does not survive a
second.** Rewriting a branch is harmless only while nobody else tracks it; the
moment someone does, every amended commit is one that disappears under their
next `pull`. `dev` is a shared-integration name by convention, so a second
writer makes it append-only exactly like `main`, and the rewriting moves to a
personal branch per unit that merges into `dev` once it settles. The layout here
is that arrangement with the middle layer collapsed, which is only correct while
the personal branch and the integration branch would hold the same commits.

(The `dev` branch and a `-dev` version suffix are unrelated; the suffix is
below.)
...
4. Install the vsix and reload — section 5. Shipping one nobody has run is how a
   ...
   cannot be replaced.
5. Commit the bump and the changelog together on `dev`, and tag that commit.
...
`X.Y.Z-dev` reads as "heading for X.Y.Z", which is also how a release that was
tagged but never uploaded is unwound: `1.1.0` back to `1.1.0-dev` leaves `1.1.0`
free to ship later, as a published number never would be. That direction is
backwards, so a local install of the bare version outranks the dev build
replacing it. `--force` re-points VS Code anyway, but the superseded folder is
left behind under `~/.vscode/extensions`, so folder names stop being evidence —
ask `code --list-extensions --show-versions` which one is live.
